    ...
    if apps_dict:
        ...
        if matches and matches[0][1] >= 80:
            ...
            try:
                ...
                return f"No encontre '{app_name}', pero abri '{best_match}'"
            except Exception as e:  # noqa: BLE001
                return f"Error al abrir '{best_match}': {e}"

    # 3) Try UWP (Microsoft Store) apps
    uwp_dict = scan_uwp_apps()
    if app_name in uwp_dict:
        appid = uwp_dict[app_name]
        try:
            subprocess.run(["explorer.exe", f"shell:AppsFolder\\{appid}"], shell=True, check=False)
            return f"Abriendo app UWP '{app_name}'"
        except Exception as e:  # noqa: BLE001
            return f"Error al abrir UWP '{app_name}': {e}"

    # 4) Fuzzy match among UWP apps
    if uwp_dict:
        ...
        if matches and matches[0][1] >= 80:
            ...
            subprocess.run(["explorer.exe", f"shell:AppsFolder\\{appid}"], shell=True, check=False)
            return f"No encontre '{app_name}', pero abri '{best_match}' (UWP)"

    return f"No se encontro ninguna aplicacion llamada '{app_name}'"
